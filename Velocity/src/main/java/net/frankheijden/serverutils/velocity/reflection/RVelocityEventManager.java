...
public class RVelocityEventManager {
    ...
    public static <E> CompletableFuture<E> fireForPlugins(
        ...
        if (registrations.isEmpty()) {
            // Optimization: nobody's listening. Also avoids touching registrations.get(0) below.
        ...
        // Velocity 4.x removed VelocityEventManager#getAsyncExecutor(): there is no longer a
        // single manager-level async executor. Each HandlerRegistration now runs on its own
        // plugin's executor (registration.plugin.getExecutorService()), same as Velocity's own
        // VelocityEventManager#fire(Object) does internally.
        if (alwaysAsync) {
